...
def simulate_genotypes_w_ld(n_sample=100, m=50000, conseq_r2=0.9, m_ld_chunk_size=100, diploid=False, verbose=False):
    ...
    (num_snps,n) = snps.shape
    assert n==n_sample, 'Problems with simulating SNPs, sample sizes do not match.'
    assert m==num_snps, 'Problems with simulating SNPs, sample sizes do not match.'
